...
```
ヘッダー：[ランキング] [配信] [タグ] [ピックアップ] [ご意見（控えめ）] [宣伝]
             ↑フェーズ1-3  ↑フェーズ7  ↑フェーズ6  ↑フェーズ5  ↑フェーズ8.5      ↑フェーズ8以降
  ├─ ランキング配下タブ：
  │     [日本で人気]（/ デフォルト）
  │     [日本の急上昇]（/trending）
  │     [カテゴリ別]（/categories）
...
### フェーズ8以降（方針転換：非課金の拡張機能）

**⚠️ 2026-04-23 方針転換**：当初設計の「有料宣伝枠」は Roblox Name and Logo Guidelines・Creator Third Party App Policy 上、商業利用条項に抵触するリスクが高いため**保留**。詳細は `higesakusei/ToS確認/03_robro-jpリスク分析.md`。

- **フェーズ8**：法令・規約対応（`/privacy` `/terms` `/contact`、削除申請フロー、フッター「公式ではない」表記、`scripts/purge-roblox-data.ts` 準備）
- **フェーズ9**：編集者ピックアップ拡張（`/featured` の記事型連載、週次更新、OG画像生成）
- **フェーズ10**：タグ職人バッジ／ランキング（accounts 最小実装、Supabase Auth でログインのみ）
- **フェーズ11**：開発者による自ゲーム紹介文セルフ登録（無料、Yuki承認制。Robloxの商業利用条項回避）
- **フェーズ12**：サイト全体でのスポンサー枠（Robloxゲーム**ではなく**、Robloxコミュニティ向けの商品・サービス広告：VTuber機材、プログラミング教室、書籍等）

---

### フェーズXX（保留）：有料宣伝機能

**保留理由**：Roblox規約上、Robloxゲームを対価を取って露出させる行為は商業利用に該当する。実装にはRobloxとの個別ライセンス契約が必要。
**再開条件**：(a) Roblox公式のパートナーシップ機会、(b) 法人化してRobloxと直接交渉できる体制、(c) サイト単独収益で自立しておりRoblox由来データに依存しない設計に転換可能。
...
- **Robloxゲームを対価で宣伝する機能**（商業利用条項抵触のため、フェーズXX保留）
